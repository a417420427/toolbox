...
import { XmlFormatter } from '@/utils/tools/xmlFormatter';
...
const sample = '<root><person><name>张三</name><age>30</age></person></root>';
...
const XmlFormatterPage: React.FC = () => {
  ...
  const process = () => {
    const r = XmlFormatter.process(input, mode === 'minify');
    setOutput(r.result);
    setError(r.error);
  };
  ...
  return (
    <View>
      <View className={toolStyles.section}>
        <Text className={toolStyles.sectionTitle}>XML 输入</Text>
        <Textarea
