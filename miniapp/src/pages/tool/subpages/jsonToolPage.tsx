import React, { useState } from 'react';
import { View, Text, Textarea, Button } from '@tarojs/components';
import { JsonTool, JsonResult } from '@/utils/tools/jsonTool';
import toolStyles from '@/styles/tool-common.module.scss';

const modes = [
  { value: 'format', label: '格式化' },
  { value: 'minify', label: '压缩' },
];

const JsonToolPage: React.FC = () => {
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [mode, setMode] = useState('format');
  const [indent, setIndent] = useState(2);
  const [error, setError] = useState<string | undefined>();

  const process = () => {
    let r: JsonResult;
    if (mode === 'format') {
      r = JsonTool.format(input, indent);
    } else {
      r = JsonTool.minify(input);
    }
    setOutput(r.result);
    setError(r.error);
  };

  const fillSample = () => {
    setInput(JsonTool.sample);
  };

  return (
    <View style={{ padding: '0 32rpx 32rpx' }}>
      <View className={toolStyles.section}>
        <Text className={toolStyles.sectionTitle}>JSON 输入</Text>
        <Textarea
          className={toolStyles.textarea}
          value={input}
          onInput={e => setInput(e.detail.value)}
          placeholder="粘贴 JSON 内容..."
          style={{ height: '200rpx', fontFamily: 'monospace', fontSize: '24rpx' }}
        />
      </View>
      <View className={toolStyles.row}>
        <View className={toolStyles.segmentedControl}>
          {modes.map(m => (
            <Text
              key={m.value}
              className={`${toolStyles.segment} ${mode === m.value ? toolStyles.segmentActive : ''}`}
              onClick={() => setMode(m.value)}
            >
              {m.label}
            </Text>
          ))}
        </View>
        {mode === 'format' && (
          <View className={toolStyles.indentPicker}>
            {[2, 4, 8].map(i => (
              <Text
                key={i}
                className={`${toolStyles.indentOpt} ${indent === i ? toolStyles.indentOptActive : ''}`}
                onClick={() => setIndent(i)}
              >缩进 {i}</Text>
            ))}
          </View>
        )}
      </View>
      <View className={toolStyles.actionRow}>
        <Button className={toolStyles.btnPrimary} onClick={process}>执行</Button>
        <Button className={toolStyles.btnSecondary} onClick={fillSample}>填入示例</Button>
      </View>
      {error && (
        <View className={toolStyles.errorBox}>
          <Text className={toolStyles.errorText}>{error}</Text>
        </View>
      )}
      {output && (
        <View className={toolStyles.section}>
          <Text className={toolStyles.sectionTitle}>输出结果</Text>
          <View className={toolStyles.resultBox}>
            <Text selectable className={toolStyles.monoText}>{output}</Text>
          </View>
        </View>
      )}
    </View>
  );
};

export default JsonToolPage;
