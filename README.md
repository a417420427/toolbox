...
| **健康指标** 🆕 | BMI / 体脂率 / 基础代谢 (BMR) / TDEE / 理想体重 | ✅ |
...
| **Markdown 预览** 🆕 | 输入 Markdown 实时渲染（纯 Dart 解析 + 文本渲染） | ✅ |
| **排序 / 去重** 🆕 | 文本行升序/降序/按长度/反转 / 去重/忽略大小写 / 统计 | ✅ |
...
| **AES 加解密** 🆕 | AES-128/192/256，CBC/ECB/CTR/GCM 模式，PBKDF2 密钥派生 | ✅ |
...
| **URL 解析器** 🆕 | 协议 / 主机名 / 端口 / 路径 / 查询参数 / Hash 逐项解析 | ✅ |

> 另有 `image_tool.dart`（图片处理）与 `memo_tool.dart`（备忘定时器）位于 `packages/flutter_shared/lib/tools/` 但尚未注册为正式工具。
